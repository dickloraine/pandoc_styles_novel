...
formats:
  ...
  - html
  - markdown
destination: ../src/styles/novel/docs
...
style-definition:
  all:
    command-line:
      ...
      toc-depth: 3
      filter:
        - ../src/styles/novel/filter/novel_poem.py
    metadata:
      poem-style: top
      lang: en
  html:
    ...
    sass:
      files:
        - ../src/styles/novel/sass/novel.scss
        - ../src/styles/novel/sass/novel_navigation.scss
      variables:
        indent: false
        nav-top: false
        nav-font-size: 11pt
  ...
  pdf:
    command-line:
      pdf-engine: xelatex
      # template: ../src/styles/novel/template/novel.tex
    metadata:
      ...
      toc-dots: false
    replace-in-template:
      - pattern: \\usepackage{amssymb,amsmath}
        replacement-text: |
          \usepackage{verse}
...
This style-pack gives you some additional ways to format your text, so that you can easily produce books for print and e-Readers.
...
You can modify many things about the output in the metadata of your book.
...
Below you find a description of each type provided with the template and which filter is used to render it. If no filter is given, it is named like the style. Some styles have options how they should look in the rendered book. You can change these in the metadata of your book.

Most styles use the **Div-Syntax**. If no syntax is given, use that syntax. If they use the **Code-Block-Syntax** this is explicitly stated.
...
- fleuron: A new scene is marked by an image. You need to provide an image in the options
...
- bottom: Title and author are positioned on the bottom right of the poem
...
- top: Positions the title on the top, regardless of the render option used

- bottom: Positions the title on the bottom, regardless of the render option used

- one-line: Positions author and title in one line on the bottom, regardless of the render option used

- noversewidth: Per default the filter calculates the best position for the poem. If you don't want this, use this option
...
- bottom: Title and author are positioned on the bottom right of the quote
